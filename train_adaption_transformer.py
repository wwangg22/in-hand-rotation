...
class EpisodeDataset:
    ...
    def _sample_one_episode(self, frames: int):
        """
        Return `frames` *contiguous* indices from a randomly‑chosen episode
        that is at least `frames` long.
        """
        # pick an episode long enough
        pool = [i for i, (cid, s, e) in enumerate(self.episodes)
                if (e - s + 1) >= frames]
        if not pool:
            raise ValueError(f"No episode ≥ {frames} frames")
        ep_idx            = random.choice(pool)
        cid, ep_start, ep_end = self.episodes[ep_idx]
        item_str          = self.episode_items[ep_idx]

        # choose a random contiguous window inside the episode
        max_offset        = (ep_end - ep_start + 1) - frames
        offset            = random.randint(0, max_offset)
        start             = ep_start + offset
        idxs              = torch.arange(start, start + frames)  # contiguous
    ...
    def _load_chunk(self, cid: int):
        ...
        return dict(
            obs         = data[0][order],
            teacher_obs = data[1][order],  # teacher_obs is same as obs
